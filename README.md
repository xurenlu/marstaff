...
[![Version](https://img.shields.io/badge/version-1.21.0--rc1-blue.svg)](CHANGELOG.md)
...
- **节点（Node）生态**：WebSocket `?role=node` 连接侧车/手机客户端，登记能力后 Agent 可通过 `node_list` / `node_invoke` 调用（需配置 `gateway_node.token`）
- **运维**：`./bin/gateway doctor` 检查配置、数据库、技能目录等；`/api/health` 返回 `nodes_online`、`nodes_enabled`
- **多会话协作**：`sessions_list` / `sessions_history` / `sessions_send` / `sessions_spawn`；`sessions_send` 支持 `wait_for_reply` 同步等待目标会话回复，并在消息 metadata 中写入 `cross_session_from`
